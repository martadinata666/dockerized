...
getRemoteVersion() {
	...
	local suite="$1"; shift # bullseye
	local dpkgArch="$1"; shift # arm64

	curl -fsSL "https://ftp.osuosl.org/pub/mariadb/repo/$version/debian/dists/$suite/main/binary-$dpkgArch/Packages" 2>/dev/null  \
		| tac|tac \
...
for version in "${versions[@]}"; do
	...
	if [ "$suite" = buster ]
	then
		sed -i 's/libjemalloc2/libjemalloc1/' "$version/Dockerfile.debian"
	fi
	...
	case "$version" in
		10.2 | 10.3 | 10.4 | 10.5) ;;
		*)
			sed -i -e '/^CMD/s/mysqld/mariadbd/' "$version/Dockerfile.debian"
